...
import { AddNewClient, ClientIntakeForm } from 'app/main/clients/clients.component';
...
export class HistoryAppointmentComponent implements OnInit {
  ...
  historyAppointment = true;
  constructor(
    @Optional() public dialogRef: MatDialogRef<HistoryAppointmentComponent>,
  ...
  ngOnInit() {
    this.spinner.show();
    let urlGenerator = `${APPOINTMENT}/search?booking_reference=${this.data.booking_reference}&location_id=${this.data.location}`;
    this._service.get(`${urlGenerator}`).subscribe(res => {
      this.AppointmentHistoryData = res;
      this.AppointmentHistoryData.forEach(item => {
        // let st = moment(item.startTime);
        // let et = moment(item.endTime);
        let duration = moment.duration(moment(item.endTime).diff(moment(item.startTime)));
        let remain_hour = duration.hours();
        let remain_min = duration.minutes();
        item['remain_hour'] = remain_hour;
        item['remain_min'] = remain_min;
        setTimeout(() => {
          this.spinner.hide();
        }, 500);
      })
    ...
    /**
     * 
     * copy from view appointment ${View-appointment}
     * 
     */
    ...
    setTimeout(() => {
      ...
      this.appointment_id = this.data._id;
